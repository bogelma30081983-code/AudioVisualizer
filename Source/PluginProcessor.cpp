    ...
    if (fifoIndex == fftSize)
    {
        ...
        for (size_t i = 0; i < fftSize; ++i)
        {
           /* complexLeft[i] = { fftBufferL[i], 0.0f };
            ...
            fftDataL[i] = fftBufferL[i];
            fftDataR[i] = fftBufferR[i];
        }
        // 3. Другу половину масиву забиваємо нулями (обов'язкова вимога JUCE для Real-FFT)
        ...
        // 4. ВИКОНУЄМО НАДІЙНИЙ ТРАНСФОРМ
        // Він безпечно перетворить звичайні float на частоти та фазу
        ...
        //    // СТАВИМО ЗАХИСТ ВІД СМІТТЯ:
        //    if (std::isnan(magL) || std::isinf(magL)) magL = 0.0f;
        //    if (std::isnan(magR) || std::isinf(magR)) magR = 0.0f;
        ...
        //}
        // 
        // 5. Збираємо амплітуду та фазу з результату
        for (size_t i = 0; i < fftSize / 2; ++i)
        {
            // У JUCE після Real-FFT реальна частина лежить в парних індексах, а уявна - в непарних
            float realL = fftDataL[i * 2];
            float imagL = fftDataL[i * 2 + 1];
            float realR = fftDataR[i * 2];
            float imagR = fftDataR[i * 2 + 1];

            // --- СПЕКТР (Амплітуда) ---
            float magL = std::sqrt(realL * realL + imagL * imagL) / (float)fftSize;
            float magR = std::sqrt(realR * realR + imagR * imagR) / (float)fftSize;

            leftFFTData[i] = juce::Decibels::gainToDecibels(magL, -100.0f);
            rightFFTData[i] = juce::Decibels::gainToDecibels(magR, -100.0f);

            // --- ФАЗА (для твоєї червоної лінії) ---
            ...
            float currentPhaseSample = phaseDiff / juce::MathConstants<float>::pi;

            // Згладжування фази
            phaseData[i] = phaseData[i] * 0.85f + currentPhaseSample * 0.15f;
        }
